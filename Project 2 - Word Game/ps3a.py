...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  9 09:38:44 2023
...
def choose_word(hand, N=100):
    ...
    max_word_len = calculate_handlen(hand)
    letter_list = hand_to_list(hand)
    bestscore = 0
    bestword = None
    for i in range(N):
        n = random.randint(0, max_word_len)
        word1 = random.sample(letter_list, n)
        word2=""
        for i in word1:
            word2+=i
        
        if ps2.is_valid_word(word2, hand, word_list):
            current_score = ps2.get_word_score(word2, max_word_len)
...
def play_mc_hand(hand, N=100):
    ...
    for i in range(N):
        ...
        word, curr_score = choose_word(hand, N=100)
        if curr_score == 0:
            break
            
        handscore+=curr_score
        wordls.append(word)
...
def play_n_mc_hand(hand, N, n):
    ...
    while simulations < n:
        simulations += 1
        curr_list, curr_score = play_mc_hand(hand, N)
        mc_hands.append(curr_list)
        mc_scores.append(curr_score)
    # IMPLEMENT ME
